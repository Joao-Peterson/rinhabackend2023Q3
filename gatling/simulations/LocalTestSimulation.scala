...
class LocalTestSimulation extends Simulation {
    ...
    setUp(
		criacaoEConsultaPessoas.inject(
			...
			rampUsersPerSec(6).to(50).during(30.seconds)
		),
		buscaPessoas.inject(
			constantUsersPerSec(2).during(25.seconds),
			rampUsersPerSec(6).to(20).during(15.seconds)
		),
		buscaInvalidaPessoas.inject(
			constantUsersPerSec(2).during(25.seconds),
			rampUsersPerSec(6).to(15).during(15.seconds)
		)
